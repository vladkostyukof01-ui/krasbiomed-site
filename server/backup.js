...
// 3) скопировать нужный файл из data/backups/ поверх data/krasbiomed.db,
...
const dbPath = path.join(__dirname, '..', 'data', 'krasbiomed.db');
...
const backupPath = path.join(backupDir, `krasbiomed-${timestamp}.db`);
...
const files = fs.readdirSync(backupDir)
  .filter(f => f.startsWith('krasbiomed-') && f.endsWith('.db'))
  .sort();
