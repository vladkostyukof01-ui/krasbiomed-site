...
app.get('/api/services', (req, res) => {
  res.json(db.prepare('SELECT id, slug, title, short_desc, icon FROM services ORDER BY sort_order').all());
});

app.get('/api/services/:slug', (req, res) => {
  const service = db.prepare('SELECT * FROM services WHERE slug = ?').get(req.params.slug);
  if (!service) return res.status(404).json({ error: 'Раздел не найден.' });
  res.json(service);
});
...
app.get('/api/price-index', (req, res) => {
  ...
  const allItems = db.prepare('SELECT id, category_id, title, dose_note, manufacturer, price_note, in_stock FROM price_items ORDER BY sort_order').all();
...
app.get('/api/licenses', (req, res) => {
  res.json(db.prepare('SELECT id, title, caption, image FROM licenses ORDER BY sort_order').all());
});

app.post('/api/leads', (req, res) => {
  const { name, phone, organization, request_type, message, consent } = req.body || {};
  ...
  db.prepare(`
    INSERT INTO leads (name, phone, organization, request_type, message, source, consent_given)
    VALUES (?, ?, ?, ?, ?, ?, 1)
  `).run(name || null, String(phone).trim(), organization || null, request_type || null, message || null, 'zayavka-form');
  res.json({ ok: true });
});
...
app.put('/api/admin/services/:id', requireAuth, (req, res) => {
  const { title, short_desc, body } = req.body || {};
  const fields = [];
  const values = [];
  if (title !== undefined) { fields.push('title = ?'); values.push(title); }
  if (short_desc !== undefined) { fields.push('short_desc = ?'); values.push(short_desc); }
...
app.put('/api/admin/price-items/:id', requireAuth, (req, res) => {
  const { title, dose_note, manufacturer, price_note, in_stock } = req.body || {};
  const fields = [];
  const values = [];
  if (title !== undefined) { fields.push('title = ?'); values.push(title); }
  if (dose_note !== undefined) { fields.push('dose_note = ?'); values.push(dose_note); }
...
app.post('/api/admin/price-items', requireAuth, (req, res) => {
  const { category_id, title, dose_note, manufacturer, price_note, in_stock } = req.body || {};
  ...
  const result = db.prepare('INSERT INTO price_items (category_id, title, dose_note, manufacturer, price_note, in_stock, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)')
    .run(category_id, title, dose_note || '', manufacturer || '', price_note || '', in_stock === false ? 0 : 1, maxOrder + 1);
...
app.get('/uslugi/:slug', (req, res) => {
  ...
  const html = template
    ...
    .replace(
      '<meta name="description" id="pageDescription" content="Иммунобиологические препараты и вакцинация в Красноярске.">',
      `<meta name="description" id="pageDescription" content="${description}">\n<link rel="canonical" href="${canonical}">\n<meta property="og:title" content="${title}">\n<meta property="og:description" content="${description}">\n<meta property="og:type" content="website">`
    );
  res.send(html);
});

app.get('/vakciny', (req, res) => {
  res.sendFile(path.join(__dirname, '..', 'public', 'vakciny.html'));
});

app.get('/dokumenty', (req, res) => {
  res.sendFile(path.join(__dirname, '..', 'public', 'dokumenty.html'));
});
...
app.get('/zayavka', (req, res) => {
  res.sendFile(path.join(__dirname, '..', 'public', 'zayavka.html'));
});
...
app.listen(PORT, () => {
  console.log(`ООО «КрасБиоМед-Иммуно» запущен: http://localhost:${PORT}`);
